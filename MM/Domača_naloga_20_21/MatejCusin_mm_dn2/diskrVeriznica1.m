function potencilna = diskrVeriznica1(w0, obesisceL, obesisceD, L, M, g)
% function potencilna = diskrVeriznica1(w0, obesisceL, obesisceD, L, M, g)
% diskrVeriznica1 resi problem diskretne veriznice: preko fsolve najde u in v, tako da
% F(u,v) = [0; 0], nato za to verižnico izračuna potencialno energijo.
...
F = @(w) F_uv(w, obesisceL, obesisceD, L, vsota_mi);
w = fsolve(F, w0);
% izracunamo x-e
...
potencilna = 0;
for i = 1 :length(M) 
    potencilna = potencilna + g * M(i) * (x(2, i) + x(2, i + 1)) / 2;
end
